...
COCO (Common Objects in Context) is a widely used dataset format for object detection and segmentation tasks. **Detectron2** expects datasets in COCO format when using `register_coco_instances()`. Adapted from [Yolo to COCO converter](https://github.com/Taeyoung96/Yolo-to-COCO-format-converter/blob/master/create_annotations.py)
